...
data{
 ...
 // main x var
  ...
  vector[nData] OverallNiDist; // dsep target
...
parameters {
  ...
  matrix[5,nCountry] z_p;  // parameter matrix for each country [param x country]

   vector<lower=0>[5] sigma_p; //sigma intercept and slope(s)
   vector[5] hyperbeta; // hyper prior intercept and slope(s)

   cholesky_factor_corr[5] L_p; // cholesky version of above
...
// Gauss intercepts
  ...
  // make model matrix
  matrix[nData, 5] M;
  matrix[5,nCountry] z; // non-centered version of beta_p
...
zUK~  normal(0,1);
 ...
 for(i in 1:nData) {
  if (CountryID[i] == 1) {
   mu[i] =  hyperbeta[1] + z[1,CountryID[i]] + (hyperbeta[2] + z[2,CountryID[i]]) * M[i,2] +  (hyperbeta[3] + z[3,CountryID[i]]) * M[i,3]+ (hyperbeta[4] + z[4,CountryID[i]]) * M[i,4]+ (hyperbeta[5] + z[5,CountryID[i]]) * M[i,5] +  spK[SiteID[i]];
  }
  if (CountryID[i] == 2) {
   mu[i] =  hyperbeta[1] + z[1,CountryID[i]] + (hyperbeta[2] + z[2,CountryID[i]]) * M[i,2] +  (hyperbeta[3] + z[3,CountryID[i]]) * M[i,3]+ (hyperbeta[4] + z[4,CountryID[i]]) * M[i,4] + (hyperbeta[5] + z[5,CountryID[i]]) * M[i,5] + finK[SiteID[i]];
  }
  if (CountryID[i] == 3) {
     mu[i] =  hyperbeta[1] + z[1,CountryID[i]] + (hyperbeta[2] + z[2,CountryID[i]]) * M[i,2] +  (hyperbeta[3] + z[3,CountryID[i]]) * M[i,3]+ (hyperbeta[4] + z[4,CountryID[i]]) * M[i,4] + (hyperbeta[5] + z[5,CountryID[i]]) * M[i,5] +  ukK[SiteID[i]];
  }
  }
