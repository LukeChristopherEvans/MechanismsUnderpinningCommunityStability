...
data{
 ...
 // main x var
  ...
  vector[nData] SynIndex ;  // dsep target
...
parameters {
  ...
  matrix[7,nCountry] z_p;  // parameter matrix for each country [param x country]

   vector<lower=0>[7] sigma_p; //sigma intercept and slope(s)
   vector[7] hyperbeta; // hyper prior intercept and slope(s)

   cholesky_factor_corr[7] L_p; // cholesky version of above
...
// Gauss intercepts
  ...
  // make model matrix
  matrix[nData, 7] M;
  matrix[7,nCountry] z; // non-centered version of beta_p
...
for(i in 1:nData) {
 if (CountryID[i] == 1) {
  mu[i] =  hyperbeta[1] + z[1,CountryID[i]] + (hyperbeta[2] + z[2,CountryID[i]]) * M[i,2] +  (hyperbeta[3] + z[3,CountryID[i]]) * M[i,3]+ (hyperbeta[4] + z[4,CountryID[i]]) * M[i,4]+ (hyperbeta[5] + z[5,CountryID[i]]) * M[i,5] +  (hyperbeta[6] + z[6,CountryID[i]]) * M[i,6] +(hyperbeta[7] + z[7,CountryID[i]]) * M[i,7] + spK[SiteID[i]];
 }
 if (CountryID[i] == 2) {
  mu[i] =  hyperbeta[1] + z[1,CountryID[i]] + (hyperbeta[2] + z[2,CountryID[i]]) * M[i,2] +  (hyperbeta[3] + z[3,CountryID[i]]) * M[i,3]+ (hyperbeta[4] + z[4,CountryID[i]]) * M[i,4] + (hyperbeta[5] + z[5,CountryID[i]]) * M[i,5] + (hyperbeta[6] + z[6,CountryID[i]]) * M[i,6] +(hyperbeta[7] + z[7,CountryID[i]]) * M[i,7] + finK[SiteID[i]];
 }
 if (CountryID[i] == 3) {
    mu[i] =  hyperbeta[1] + z[1,CountryID[i]] + (hyperbeta[2] + z[2,CountryID[i]]) * M[i,2] +  (hyperbeta[3] + z[3,CountryID[i]]) * M[i,3]+ (hyperbeta[4] + z[4,CountryID[i]]) * M[i,4] + (hyperbeta[5] + z[5,CountryID[i]]) * M[i,5] +  (hyperbeta[6] + z[6,CountryID[i]]) * M[i,6] +(hyperbeta[7] + z[7,CountryID[i]]) * M[i,7] + ukK[SiteID[i]];
 }
 }
