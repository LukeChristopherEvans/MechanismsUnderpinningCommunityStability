...
smalllist = list(
  ...
  x =  cbind(CommunityList$LgMeanAb,CommunityList$SppRich)
)
...
setwd("")
tiff("cor6LgAbSpRi.tif", res=600, compression = "lzw", height=6, width=6.5, units="in")
stan_dens(d1,"rho")+ xlim(-1,1) +xlab("Log mean abundance vs Species richness")
dev.off()
