...
smalllist = list(
  ...
  x =  cbind(CommunityList$LgMeanCom,CommunityList$SppRich)
)
...
setwd("")
tiff("cor5LgCommSpRi.tif", res=600, compression = "lzw", height=6, width=6.5, units="in")
stan_dens(d1,"rho")+ xlim(-1,1) +xlab("Log mean community abundance vs Species richness")
dev.off()
